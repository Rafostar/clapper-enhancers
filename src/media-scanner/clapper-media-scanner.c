...
static void
_source_setup_cb (GstElement *urisourcebin, GstElement *source, ClapperMediaScanner *self)
{
  ...
  GST_INFO_OBJECT (self, "Source setup: %s", factory_name);
...
clapper_media_scanner_thread_start (ClapperThreadedObject *threaded_object)
{
  ...
  g_signal_connect (self->urisourcebin, "source-setup", G_CALLBACK (_source_setup_cb), self);
