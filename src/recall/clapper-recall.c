...
clapper_recall_marker_new (gdouble position)
{
  return clapper_marker_new (RECALL_MARKER_TYPE,
...
_generate_data_hash (ClapperRecall *self, ClapperRecallMemo *memo)
{
  ...
  if (proto_found) {
    ...
    /* NOTE: 0.8 since last seek is to 20% of file size */
...
_refresh_marker_presence (ClapperRecall *self, ClapperRecallMemo *memo, gboolean forced)
{
  ...
  if (forced || has_marker != should_have_marker) {
    ...
    if (should_have_marker) {
      memo->marker = clapper_recall_marker_new (memo->position);
...
_on_hash_filled_cb (ClapperRecallData *data)
{
  ...
  if (self->persistent_storage && _ensure_db (self)) {
    if (!self->recall_stmt) {
...
clapper_recall_class_init (ClapperRecallClass *klass)
{
   ...
   *
   * Automatically seek to last remembered position.
   */
  param_specs[PROP_AUTO_RESUME] = g_param_spec_boolean ("auto-resume",
      "Auto Resume", "Automatically seek to last remembered position", DEFAULT_AUTO_RESUME,
