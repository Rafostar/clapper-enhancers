...
static gchar *
generate_hash (ClapperRecall *self, ClapperMediaItem *item)
{
  const gchar *uri = clapper_media_item_get_uri (item);
  gchar *hash = NULL;

  if (g_str_has_prefix (uri, "file://")) {
    GST_FIXME_OBJECT (self, "Generate hash from file data as first method");
  }
  if (!hash) {
  ...
  GST_TRACE_OBJECT (self, "Generated %" GST_PTR_FORMAT " hash: %s", item, hash);

  return hash;
}

static void
memorize_played_item_position (ClapperRecall *self)
{
  GST_LOG_OBJECT (self, "Memorize");

  if (!self->played_hash || !_ensure_db (self))
    return;
  ...
  sqlite3_bind_text (self->memorize_stmt, 1, self->played_hash, -1, SQLITE_STATIC);
  sqlite3_bind_int64 (self->memorize_stmt, 2, CLAPPER_RECALL_SECONDS_TO_USECONDS (self->played_position));
  ...
  /* Truncate if accumulated a lot of changes */
  /* FIXME: Run on idle */
  ...
  GST_INFO_OBJECT (self, "%" GST_PTR_FORMAT " memorized position: %lf",
      self->played_item, self->played_position);
}

static gdouble
recall_position (ClapperRecall *self, ClapperMediaItem *item)
{
  gchar *hash = NULL;
  gdouble position = -1;
  ...
  sqlite3_bind_text (self->recall_stmt, 1, (hash) ? hash : self->played_hash, -1, SQLITE_STATIC);
  ...
  GST_INFO_OBJECT (self, "%" GST_PTR_FORMAT " recalled position: %lf", item, position);

  return position;
}

/* transfer-full */
static ClapperMarker *
find_marker (ClapperRecall *self, ClapperMediaItem *item)
{
...
static void
insert_marker (ClapperRecall *self, ClapperMediaItem *item, gdouble position)
{
  ClapperTimeline *timeline = clapper_media_item_get_timeline (item);
  ...
  recall_marker = clapper_marker_new (RECALL_MARKER_TYPE,
      "Resume point", position, position);
...
static void
remove_marker (ClapperRecall *self, ClapperMediaItem *item, ClapperMarker *marker)
{
  ClapperTimeline *timeline = clapper_media_item_get_timeline (item);
...
clapper_recall_state_changed (ClapperReactable *reactable, ClapperPlayerState state)
{
  ...
  if (!self->resume_done && state >= CLAPPER_PLAYER_STATE_PAUSED) {
    if (self->auto_resume && self->recalled_position > 0) { // FIXME: > min_allowed
      ClapperPlayer *player = clapper_reactable_get_player (reactable);

      if (G_LIKELY (player != NULL)) {
        clapper_player_seek (player, self->recalled_position);
...
clapper_recall_played_item_changed (ClapperReactable *reactable, ClapperMediaItem *item)
{
  ...
  if (self->played_item) {
    ...
    memorize_played_item_position (self);

    /* Remove absolete marker */
    if ((marker = find_marker (self, self->played_item))) {
      remove_marker (self, self->played_item, marker);
      gst_object_unref (marker);
    }

    /* Optionally insert updated marker */
    if (self->mark_position) // FIXME: && within min/max allowed
      insert_marker (self, self->played_item, self->played_position);
  }
  ...
  g_free (self->played_hash);
  self->played_hash = generate_hash (self, self->played_item);
  self->played_position = 0;

  /* Find marker within new played item */
  if ((marker = find_marker (self, self->played_item))) {
    had_marker = TRUE;
    self->recalled_position = clapper_marker_get_start (marker);
    gst_object_unref (marker);
  } else {
    self->recalled_position = recall_position (self, self->played_item);
  }

  if (!had_marker && self->mark_position && self->recalled_position > 0) // FIXME: && within min/max allowed
    insert_marker (self, self->played_item, self->recalled_position);
...
clapper_recall_queue_item_added (ClapperReactable *reactable, ClapperMediaItem *item, guint index)
{
  ...
  recalled_position = recall_position (self, item);

  /* Find marker within new played item */
  if ((marker = find_marker (self, item))) {
    gdouble marker_position = clapper_marker_get_start (marker);

    /* Check if marker needs updating */
    if (!(had_marker = G_APPROX_VALUE (recalled_position, marker_position, FLT_EPSILON)))
      remove_marker (self, item, marker);

    gst_object_unref (marker);
  }

  if (!had_marker && self->mark_position && recalled_position > 0) // FIXME: && within min/max allowed
    insert_marker (self, item, recalled_position);
}
    ...
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_MARK_POSITION:
      self->mark_position = g_value_get_boolean (value);
      break;
...
clapper_recall_class_init (ClapperRecallClass *klass)
{
   ...
   *
   * Whether to automatically seek to last remembered position.
   */
  param_specs[PROP_AUTO_RESUME] = g_param_spec_boolean ("auto-resume",
      "Auto Resume", "Whether to automatically seek to last remembered position", DEFAULT_AUTO_RESUME,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | CLAPPER_ENHANCER_PARAM_GLOBAL);
